...
final public class CodeVerification500CardViewController: UIViewController, Themeable {
	...
	private lazy var titleLabel: UILabel = {
		...
		label.text = HCL10n.Auth.Code500.title
		return label
	}()
	...
	private lazy var descriptionLabel: UILabel = {
		...
		let text = HCL10n.Auth.Code500.description
	...
	private lazy var retryButton: UIButton = {
		...
		button.setTitle(HCL10n.Auth.Code500.retryButtonTitle, for: .normal)
	...
	private lazy var cancelButton: UIButton = {
		let button = ThemeRoundedButton(withSelectors: [.primary, .plain])
		button.setTitle(HCL10n.Auth.Code500.cancelButtonTitle, for: .normal)
		button.snp.makeConstraints { $0.height.equalTo(40) }
		button.addTarget(self, action: #selector(didTapCancel), for: .touchUpInside)
		return button
	}()

	init(onRetry: (() -> Void)?, onCancel: (() -> Void)?) {
	...
	private func setupUI() {
		...
		let buttonsContainer = UIStackView(arrangedSubviews: [
			HCSpacerView(nil, .horizontal), cancelButton, HCSpacerView(8, .horizontal), retryButton
		])
