...
class ScheduledTaskManager : NSObject {
	...
	func setup() {
		...
		wifiMonitorQueue = DispatchQueue(label: "com.personalCloudFiles.scheduled_task_mgr.wifi_monitor")
		...
		BGTaskScheduler.shared.register(forTaskWithIdentifier: "com.personalCloudFiles.background-refresh-task", using: nil) { (task) in
...
extension ScheduledTaskManager {
	...
	func scheduleBackgroundRefreshTask() {
        ...
        let request = BGAppRefreshTaskRequest(identifier: "com.personalCloudFiles.background-refresh-task")
